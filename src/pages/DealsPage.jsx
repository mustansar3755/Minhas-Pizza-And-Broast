...
function DealCard({ deal, accentColor, glowColor }) {
  return (
    <div className="menu-card p-6 flex flex-col gap-5 relative overflow-hidden">
      ...
      <div className="flex items-end justify-between pt-2 border-t border-brand-border">
        <div>
          <p className="text-[#555] text-xs mb-0.5">Total Price</p>
          <p className="font-bold text-2xl" style={{ fontFamily: 'Playfair Display, serif', color: accentColor }}>
            Rs. {deal.price.toLocaleString()}
          </p>
        </div>
        <a href={`https://wa.me/${CONTACT.whatsapp}?text=Hi! I want to order ${deal.name}`}
...
function DealsGrid({ deals, accentColor, glowColor }) {
  const ref = useStaggerChildren(0.1)
  return (
    <div ref={ref} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
      {deals.map(deal => <div key={deal.id}><DealCard deal={deal} accentColor={accentColor} glowColor={glowColor} /></div>)}
    </div>
  )
}

export default function DealsPage() {
  const [active, setActive] = useState('dhamka-zinger')
  const titleRef = useFadeUp({ y: 30 })
  const style = TAB_STYLE[active]

  return (
    <>
      <div className="min-h-screen pt-28 pb-24" style={{ background: 'var(--color-brand-charcoal)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div ref={titleRef} className="text-center mb-14 opacity-0">
            ...
            <h1 className="text-5xl sm:text-6xl font-bold text-white mb-4"
              ...
              Hot <span className="gradient-text-red">Deals</span>
            </h1>
            <p className="text-[#666] text-base max-w-xl mx-auto mt-3">
              Budget-friendly meal combinations for students, families, and late-night cravings.
            </p>
          ...
          <div className="flex flex-wrap justify-center gap-3 mb-10">
            {Object.entries(DEALS).map(([key, cat]) => (
              <button key={key} onClick={() => setActive(key)}
                ...
                style={active === key ? { boxShadow: `0 6px 20px ${TAB_STYLE[key].glow}` } : {}}>
                <span>{cat.icon}</span><span>{cat.label}</span>
              </button>
            ))}
          </div>

          <DealsGrid key={active} deals={DEALS[active].items} accentColor={style.text} glowColor={style.glow} />
        </div>
      </div>
      <Footer />
    </>
